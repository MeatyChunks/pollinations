...
def find_merged_weekly_pr(
    github_token: str, owner: str, repo: str, week_end: str
) -> Optional[int]:
...
def read_weekly_file(
    path: str, github_token: str, owner: str, repo: str
) -> Optional[Dict]:
...
def main():
    ...
    # Twitter
    twitter_data = read_weekly_file(
        f"{weekly_dir}/twitter.json", github_token, owner, repo
    )
    if twitter_data:
        print("  Twitter...")
    ...
    # LinkedIn
    linkedin_data = read_weekly_file(
        f"{weekly_dir}/linkedin.json", github_token, owner, repo
    )
    if linkedin_data:
        print("  LinkedIn...")
    ...
    # Instagram
    instagram_data = read_weekly_file(
        f"{weekly_dir}/instagram.json", github_token, owner, repo
    )
    if instagram_data:
        print("  Instagram...")
    ...
    if vps_host and vps_user and vps_ssh_key:
        ...
        reddit_data = read_weekly_file(
            f"{weekly_dir}/reddit.json", github_token, owner, repo
        )
        if reddit_data:
            print("  Reddit...")
    ...
    # Discord
    discord_data = read_weekly_file(
        f"{weekly_dir}/discord.json", github_token, owner, repo
    )
